...
##      own `@model` submodel that the composer accepts as an injectable
##      keyword. Tests in `test_models_v2.jl` enforce this.
##   2. EVERY delay is specified via a prior. No delay distribution and
##      no generation time is a fixed constant. The four time scales
##      consumed by the observation expectations — incubation
##      (`incubation_v2`), onset-to-death (`onset_to_death_v2`),
##      onset-to-report (`onset_to_report_v2`) and onset-to-detection
##      (`detection_window_v2`) — each sample their parameter(s) from a
##      prior in their own submodel. The growth rate `r` is itself
##      sampled via `τ` and `m` from `growth_v2`, so the timescale of
##      spread is not a fixed input either.
...
# and return a Distribution. Injected into the observation submodels so
# callers can swap them without editing the submodel.
_default_poisson_obs(μ) = Poisson(max(μ, eps(typeof(μ))))
_default_nbinomial_obs(k, μ) = _safe_nbinomial_v2(k, μ)
...
Anchored on the bdbv-linelist Bayesian reanalysis of the Isiro 2012
line list:
...
The lower bounds (`α ≥ 1`, `θ ≥ 10^{-3}`) match `incubation_v2` and
`onset_to_report_v2` so an extreme NUTS proposal cannot push the
sampled `Gamma(α, θ)` toward the `α → 0` density spike, where AD on
the density itself becomes ill-behaved (matches the coderabbit-flagged
consistency note).
"""
...
Onset-to-detection window building block for v2. Now that incubation
is modelled separately, `w` is unambiguously onset-to-detection (the
current model's window mixes incubation and detection together). The
prior is centred at 7 d with SD 3 d:
...
invariant.
"""
@model function detection_window_v2(;
...
Genetic TMRCA soft lower-bound building block for v2. Same
construction as the current model's `genetic_seeding_model`:
observing the molecular-clock TMRCA read `tmrca_days` at the upper
censoring point of `Normal(T, σ)` contributes a one-sided likelihood
...
`T` is the elapsed-time deterministic from `growth_v2`; `tmrca_days`
and `tmrca_days_sd` come from the observation block.
"""
@model function genetic_seeding_v2(T, tmrca_days::Real;
...
[`OnsetIncidence`](@ref) `oi`, the Uganda ascertainment `p_uganda`,
the per-capita travel rate `q` and the onset-to-detection window `w`,
and ties the exported-case count to the latent state through
[`expected_exports_v2`](@ref). The observation distribution is
injected via the `obs` keyword: it must accept the expected count
`μ_e` and return a Distribution. Defaults to a Poisson, matching the
small-detection-probability regime of the export count.
"""
...
convolution through [`expected_deaths_v2`](@ref), and applies an
injectable observation distribution. `obs` takes the shared dispersion
`k` and the expected count `μ_d` and returns a Distribution; the
default uses the NaN-safe NegBinomial that the rest of the package
shares. `CFR` is the fraction of onsets that die.
"""
...
v2 reported-cases observation submodel. Builds the onset-to-report
CDF holder once, forms the delayed-report convolution through
[`expected_reports_v2`](@ref), and applies an injectable observation
distribution. Defaults to the same shared-dispersion NegBinomial as
the deaths stream. Unlike the current model the reporting is delayed:
recent infections are not yet fully ascertained.
"""
...
incubation, onset-to-death delay, onset-to-report delay,
onset-to-detection window, CFR, traveller volume, dispersion, pooled
ascertainment), tabulates the onset-incidence curve once per draw
with [`OnsetIncidence`](@ref), and threads it into the three
...
are injectable so a single-stream variant or a sensitivity analysis
can swap a prior without editing this composer:

| Keyword | Default | What it controls |
|---|---|---|
| `growth` | (required) | growth rate / elapsed time |
| `incubation` | `incubation_v2()` | infection→onset delay |
| `death` | `onset_to_death_v2()` | onset→death delay |
| `report` | `onset_to_report_v2()` | onset→report delay |
| `window` | `detection_window_v2()` | onset→detection window |
...
| `genetic` | `genetic_seeding_v2` | TMRCA term; `nothing` to drop |
"""
@model function bvd_joint_v2(
        ...
        window     = detection_window_v2(),
        genetic    = genetic_seeding_v2,
    ...
    exports_state ~ to_submodel(
        exports_obs_v2(exported_cases, oi, p_uganda, q, w;
            obs = exports_obs), false)
    deaths_state ~ to_submodel(
        deaths_obs_v2(total_deaths, oi, death_state.dist, CFR, k;
            obs = deaths_obs), false)
    cases_state ~ to_submodel(
        cases_obs_v2(reported_cases, oi, report_state.dist, p_drc, k;
            obs = cases_obs), false)
