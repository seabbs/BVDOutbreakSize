...
Status: candidate redesign on the `arch-state-space` branch.
Additive to the existing model; the deterministic exponential-growth baseline is unaffected.

## Motivation and contrast with the sibling tracks
...
Two sibling tracks already address this gap.

- The renewal architecture (`arch-renewal`, PR #103) replaces `exp(r·s)` with a deterministic discrete-time renewal `I_t = R_t · sum(I_{t-s} · g_s)` driven by a weekly random-walk `R_t`.
The latent infection process is still deterministic given `R_t`; only `R_t` carries the noise.
- The stochastic-latent track (`arch-stochastic`, PR #101) adds Gaussian process noise to `log C(s)`, i.e. a log-Gaussian / linear-noise approximation (LNA) of the underlying stochastic infection process.
...
### Observation staging — same as the convolution / renewal route
...
This is the same onset-staging the discrete-renewal architecture uses; it makes detection and death consistently timed from onset.

### Observation likelihoods

Observations condition on the available *cumulative* totals through to the data cut-off, exactly as the baseline model does:
...
The cumulative version is demonstrated end-to-end here; the daily version is a drop-in replacement on the same vectors.
...
Because `n` is fixed here (set by the daily-grid length), the bound is effectively a one-sided likelihood on `n`.
...
Mirroring the main model and the renewal architecture:
...
- Delay discretisation by hand-rolled trapezoidal integration over daily bins with the density at zero forced to zero, the same AD-safe trick the renewal architecture uses (CensoredDistributions' analytical primary-censored CDF is not Mooncake-differentiable; see the renewal track's verdict).
...
The implementation here covers the full generative model and demonstrates:
...
For `n ≈ 130 days` and `N = 200` particles (a defensible minimum for a moderately overdispersed daily process before the particle population collapses), one Gibbs sweep is roughly `n · N ≈ 26000` forward-model evaluations of the daily kernel, against a single forward-model evaluation per NUTS leapfrog in the LNA track.
A NUTS step typically does 10-200 leapfrog evaluations, so one PG-NUTS sweep costs roughly `10-100×` a single NUTS step in the LNA track.
Translated to wall clock, expect:

- LNA track full fit: ~15-25 min (per its own measurements).
...
## Comparison to the LNA track (`arch-stochastic`, #101)
...
The LNA track shows that the censored upper-tail construction is what makes the earliest onset usable.
...
The honest read-out: the LNA track gets most of what the exact discrete process gives, at much lower cost, because all the available observations are aggregates.
...
- `scripts/state_space.jl` — driver: prior-predictive sample, gradient check on the continuous block, short particle-Gibbs run, console summary.
- `docs/src/proposals/state-space-particle.md` — this proposal.
...
The cost is roughly an order of magnitude or more above the LNA track, and the inferential pay-off on the available data is small because all the observations are aggregates that the LNA already captures faithfully.

Recommendation: do not pursue this as the default architecture for the current data.
The LNA track (`arch-stochastic`, #101) is the right route for production while the streams remain aggregate.
The state-space architecture here is worth keeping as a reference point: if a future data drop adds genuinely time-resolved early-phase records (a day-by-day first-onset trail, or sequenced cases with sampling dates that anchor the early branching), the discreteness of the latent path starts to matter, and the implementation here is the seed of the right route at that point.
